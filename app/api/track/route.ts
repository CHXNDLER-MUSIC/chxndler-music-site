...
export async function POST(req: NextRequest) {
  try {
    ...
    // touch the session (function lives in analytics schema)
    const { error: rpcError } = await supabaseAdmin.rpc('analytics.touch_session', {
      p_session_id: body.session_id,
      p_user_agent: user_agent,
      p_ip_hash: ip_hash,
    });
    if (rpcError) {
      console.error('touch_session error:', rpcError);
      // continue anyway; event insert can still succeed
    }

    // insert the event (client already set to db schema 'analytics')
