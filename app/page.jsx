...
/*  CHXNDLER — Cockpit WOW (v3.0)
    - Neon glass HUD with bolts + glow
    - Cockpit overlay + parallax background
    - Unified glass buttons + social pills
    ...
    <main className="ck-root">
      ...
      <style jsx>{`
        .ck-root { position:relative; width:100vw; height:100vh; overflow:hidden;
          background:radial-gradient(120% 120% at 50% -10%,#0b1020 0%,#06080f 55%,#04050a 100%); color:#fff;
        ...
        .ck-socials { position:absolute; left:16px; top:50%; transform:translateY(-50%);
          display:flex; flex-direction:column; gap:12px; }
        .ck-pill { display:inline-flex; align-items:center; justify-content:center;
          height:38px; width:140px; font-size:13px; font-weight:800; color:#fff; text-decoration:none;
          background:rgba(28,34,48,.5); border:1px solid rgba(255,255,255,.16); border-radius:14px;
          box-shadow:0 12px 28px rgba(0,0,0,.38), inset 0 0 12px rgba(56,182,255,.14);
          backdrop-filter:blur(6px); transition:background .18s, box-shadow .18s; }
        .ck-pill:hover { background:rgba(255,255,255,.22); box-shadow:0 14px 38px rgba(0,0,0,.44),0 0 20px rgba(56,182,255,.18); }

        .ck-hud-wrap { position:absolute; left:50%; top:56px; transform:translateX(-50%); width:92vw; max-width:720px; perspective:1000px; }
        .ck-hud { position:relative; border-radius:24px; padding:14px 16px; transform:rotateX(2deg);
        ...
        .ck-bolt { position:absolute; top:-6px; width:10px; height:10px; border-radius:999px;
          background:rgba(255,255,255,.35); box-shadow:0 0 10px rgba(255,255,255,.38); animation:boltGlow 2.8s infinite; }
        .ck-bolt-left{ left:18px; } .ck-bolt-right{ right:18px; animation-delay:.9s; }
        @keyframes boltGlow {0%,100%{opacity:.65}50%{opacity:1}}

        .ck-row{display:flex;align-items:center;gap:12px}.ck-gap{margin-top:8px;gap:10px}
        .ck-flex1{flex:1;min-width:0}
        .ck-title{font-size:16px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 2px 14px rgba(0,0,0,.55)}

        .ck-btn{padding:8px 12px;min-width:44px;font-size:13px;font-weight:800;color:#fff;cursor:pointer;
        ...
        .ck-cover{width:64px;height:64px;border-radius:10px;object-fit:cover;box-shadow:0 2px 12px rgba(0,0,0,.45)}
        ...
        .ck-progress-fill{height:100%;width:0%;border-radius:999px;
          ...
          box-shadow:0 0 16px rgba(56,182,255,.35);transition:width 600ms ease}
        .ck-progress-on{width:45%}

        .ck-badge{position:absolute;bottom:6px;left:10px;font-size:11px;font-weight:800;
