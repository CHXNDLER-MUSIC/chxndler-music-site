...
function PhotoMockup({ src, alt, onError }: { src: string; alt: string; onError?: () => void }) {
  return (
    <div className="mx-auto w-full max-w-[26rem] aspect-square rounded-[1rem] overflow-hidden shadow-[0_2rem_4rem_-1rem_rgba(0,0,0,0.6)] bg-[#111113] flex items-center justify-center">
...
function CssCoverArtStage({ stage, src, alt }: { stage: "cd" | "vinyl"; src: string; alt: string }) {
  if (stage === "cd") {
    return (
      <div
        className="relative mx-auto w-full max-w-[26rem] aspect-square rounded-full overflow-hidden shadow-[0_2rem_4rem_-1rem_rgba(0,0,0,0.6)]"
...
function CassettePlaceholder({ accent }: { accent: string }) {
  return (
    <div
      className="mx-auto w-full max-w-[26rem] aspect-square rounded-[1rem] overflow-hidden shadow-[0_2rem_4rem_-1rem_rgba(0,0,0,0.6)] bg-[#161616] flex flex-col items-center justify-center gap-[0.75rem]"
      aria-hidden="true"
    >
...
function CoverArtStage({
  ...
  if (stage === "cover") {
    return (
      <div className="mx-auto w-full max-w-[26rem] aspect-square rounded-[1rem] overflow-hidden shadow-[0_2rem_4rem_-1rem_rgba(0,0,0,0.6)]">
        <img src={src} alt={alt} className="w-full h-full object-cover" />
      </div>
    );
  }
  ...
  if (stage === "cd") {
    return cd?.src ? <PhotoMockup src={cd.src} alt={alt} onError={cd.onError} /> : <CssCoverArtStage stage="cd" src={src} alt={alt} />;
  }

  if (stage === "vinyl") {
    return vinyl?.src ? (
      <PhotoMockup src={vinyl.src} alt={alt} onError={vinyl.onError} />
    ) : (
      <CssCoverArtStage stage="vinyl" src={src} alt={alt} />
    );
  }
...
export default function CoverArtViewer({
  ...
  return (
    <>
      ...
      >
        <img src={src} alt={`${songTitle} cover art`} className="w-full h-full object-cover" onError={onError} />
        <span
