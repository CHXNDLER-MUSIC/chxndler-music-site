...
export default function StudioNav() {
  ...
  return (
    <header
      className="fixed top-0 inset-x-0 z-50 flex items-center justify-between gap-[1rem] px-[6vw] sm:px-[8vw] py-[1.125rem] sm:py-[1.25rem] backdrop-blur-md bg-black/35 border-b border-white/[0.06]"
      style={{ color: "#ffffff" }}
    >
      <a
        href="#hero"
        ...
        className="flex-shrink-0 font-bold tracking-[0.2em] text-[0.8125rem] sm:text-[0.875rem] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[0.25rem] rounded-sm"
      ...
      <nav aria-label="Studio sections" className="hidden sm:flex items-center gap-[2rem]">
        {LINKS.map((link) => (
          <a
            key={link.id}
            href={`#${link.id}`}
        ...
        <a
          ...
          onMouseEnter={playHover}
          className="inline-flex items-center rounded-full border border-white/20 px-[1.125rem] py-[0.5rem] text-[0.6875rem] font-bold tracking-[0.15em] uppercase text-white/90 transition-colors duration-200 hover:border-[#ff3ea5] hover:text-[#ff3ea5] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[0.25rem]"
          style={{ outlineColor: STUDIO_PINK }}
        >
          Start a Project
        </a>
      ...
      {/* Mobile: wordmark + a single clear action — the full link list is
          one thumb-scroll away regardless, so it isn't duplicated here. */}
      <a
        href="#start-a-project"
        onClick={go("start-a-project")}
