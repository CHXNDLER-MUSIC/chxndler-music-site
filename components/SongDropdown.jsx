...
export default function SongDropdown({ items = [], initialActiveId, onChange, currentId }) {
  ...
  useEffect(() => {
    function onDocClick(e) {
      ...
      // Check if click is within the dropdown list (portal) by element containment
      if (listRef.current && listRef.current.contains(e.target)) return;
      // Extra guard: treat clicks within the list's bounding box as inside
      // This ensures clicks on the native scrollbar (not part of DOM) don't close the menu
      if (listRef.current) {
    ...
    <div ref={rootRef} className="w-full relative z-[99999] mt-7" style={{ pointerEvents: 'auto' }}>
      ...
      {open && mounted && triggerRect && typeof window !== 'undefined' ? createPortal(
        ...
        >
          ...
          {/* Native scrollbar only; custom overlay removed */}
        </div>,
        document.body
      ) : null}
      {/* Click SFX */}
